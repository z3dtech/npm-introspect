...
window.onload = function() {
    ...
    d3.json('data.json', function(err, data) {
        ...
        const usageData = data.map((d) => {
            ...
            const createPaths = g.append('g').attr('class', 'lineGraph').selectAll('path').data(scoreData). //array of array
            enter().append('path').on('click', (d) => {
                //toggle class visible, invisible
                displayData(d.name)
            }).attr('transform', 'translate(' + [0, 0] + ')').attr('class', (d) => {
                return d.name
            }).attr('d', (d) => {
                return path(d.score)
            })

            const verticalAxis = g.append('g').attr('class', 'axis').selectAll('axis').data(scoreData[0].score).enter().each(function(d, i) {
